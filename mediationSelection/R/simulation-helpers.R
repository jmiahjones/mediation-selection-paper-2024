...
.train_sl_mu <- function(X, D, M, Y,
  ...
  CONT_ENSEMBLE_METHOD <- "method.NNLS"
  CONT_ENSEMBLE_METHOD_2 <- "method.CC_LS"
  BIN_ENSEMBLE_METHOD <- "method.NNloglik"
  BIN_ENSEMBLE_METHOD_2 <- "method.CC_nloglik"
  
  if(!estD)
    stop("Must estimate D for passing the propensity through!")
