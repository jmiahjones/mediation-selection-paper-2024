...
main <- function(
  ...
  if(small.setting){
    ...
    alphas = 4*n^(-1/2)*c(1, 1, n^(1/4), rep(0, num_noise)) # D-M coef
    betas = c(1, 1, n^(-1/4), rep(0,num_noise)) # M-Y coef
  ...
  simulationsfile <- paste0("./cache/simulations/sims-", savefile_suffix, ".qs")
  if(use_sl)
    mlfile <- paste0("./cache/muhats/muhats-", savefile_suffix, ".qs")
  ...
  if(file.exists(simulationsfile)) {
    
    print("Simulations found. Loading from disk...")
    simulations <- qs::qread(file=simulationsfile, nthreads = cores)
    
    stopifnot(length(simulations) == num_simulations,
              nrow(simulations[[1]]) == n)
    print("Simulations loaded. Continuing...")
    ...
    qs::qsave(simulations, file=simulationsfile)
  }
  ...
  if(use_sl){
    ...
    if(file.exists(mlfile)) {
      ...
      print("Trained SL found. Loading from disk...")
      mu.hats <- qs::qread(file=mlfile, nthreads = cores)
      stopifnot(length(mu.hats) == num_simulations)
      print("Trained SL loaded. Continuing...")
  ...
  results <- foreach(sim.idx=1:num_simulations, sim=simulations,
    ...
    prd_results <- adp_results <- full_results <- oracle_results <- NULL
    print("Beginning prd")
